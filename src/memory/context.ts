 ...
 * - FLIPAGENT.md and project context loading
...
const logger = createLogger('memory-context');
...
export interface ContextConfig {
  ...
  /** Project root for FLIPAGENT.md discovery */
  projectRoot?: string;
  /** Custom instruction file paths to check */
 ...
 * Searches:
 * - ~/.flipagent/FLIPAGENT.md (global)
 * - Project root FLIPAGENT.md
 * - .flipagent/FLIPAGENT.md in project
 * - Custom paths
 */
export function discoverProjectInstructions(projectRoot?: string, customPaths?: string[]): ProjectContext {
  const context: ProjectContext = {};
  const searchPaths: string[] = [];

  // Global instructions
  const globalInstructions = join(homedir(), '.flipagent', 'FLIPAGENT.md');
  searchPaths.push(globalInstructions);

  // Project-level instructions
  if (projectRoot) {
    searchPaths.push(
      join(projectRoot, 'FLIPAGENT.md'),
      join(projectRoot, '.flipagent', 'FLIPAGENT.md'),
      join(projectRoot, 'docs', 'FLIPAGENT.md')
    );
  }
  ...
  // Find first existing instructions file
  for (const searchPath of searchPaths) {
    if (existsSync(searchPath)) {
      try {
        context.instructions = readFileSync(searchPath, 'utf-8');
        context.instructionsPath = searchPath;
        logger.debug({ path: searchPath }, 'Loaded project instructions');
        break;
      } catch (err) {
        logger.warn({ path: searchPath, error: err }, 'Failed to read instructions file');
      }
    }
  }
...
export interface ContextManager {
  ...
  /** Load project context (FLIPAGENT.md, etc.) */
...
export function createContextManager(
  ...
  async function summarizeMessages(messages: Message[]): Promise<string> {
    ...
    const inputTokenLimit = Math.max(1, Number(process.env.FLIPAGENT_SUMMARY_INPUT_TOKENS) || 4000);
    const maxDepth = 3;
  ...
  const manager: ContextManager = {
    ...
    buildSystemPrompt(promptConfig: SystemPromptConfig = {}) {
      ...
      // Project context (FLIPAGENT.md)
      if (promptConfig.includeProjectContext && projectContext?.instructions) {
        parts.push('\n# Project Instructions (from FLIPAGENT.md)');
        parts.push(projectContext.instructions);
      }
    ...
    loadProjectContext(projectRoot: string) {
      projectContext = discoverProjectInstructions(projectRoot, config.instructionPaths);
      state.projectContext = projectContext.instructions;
